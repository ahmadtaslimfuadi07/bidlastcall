...
class AddMoreDetailsScreen extends StatefulWidget {
  ...
  const AddMoreDetailsScreen({super.key, this.isEdit, this.mainImage, this.otherImage});
    ...
    return BlurredRouter(
      builder: (context) {
        return BlocProvider.value(
          value: (args?['context'] as BuildContext).read<FetchCustomFieldsCubit>(),
  ...
  @override
  CloudState<AddMoreDetailsScreen> createState() => _AddMoreDetailsScreenState();
}
...
class _AddMoreDetailsScreenState extends CloudState<AddMoreDetailsScreen> {
  ...
  void initState() {
    Future.delayed(
      Duration.zero,
      () {
        print("context.read<FetchCustomFieldsCubit>().getFields()****${context.read<FetchCustomFieldsCubit>().getFields().length}");
        moreDetailDynamicFields = context.read<FetchCustomFieldsCubit>().getFields().map((field) {
          ...
          if (widget.isEdit == true) {
            ...
            CustomFieldModel? matchingField = item.customFields!.any((e) => e.id == field.id) ? item.customFields?.firstWhere((e) => e.id == field.id) : null;
            ...
            child: UiUtils.buildButton(
              ...
              onPressed: () {
                if (_formKey.currentState?.validate() ?? false) {
                  ...
                  itemDetailsScreenData['custom_fields'] = json.encode(AbstractField.fieldsData);
...
// itemDetailsScreenData
                  ...
                  Navigator.pushNamed(
                    ...
                    arguments: {"isEdit": widget.isEdit == true, "mainImage": widget.mainImage, "otherImage": widget.otherImage},
                  ).then((value) {
                    screenStack--;
              ...
              child: SingleChildScrollView(
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text("giveMoreDetailsAboutYourAds".translate(context)).size(context.font.large).bold(weight: FontWeight.w600),
