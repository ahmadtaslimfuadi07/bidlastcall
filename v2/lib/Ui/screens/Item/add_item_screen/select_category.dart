...
class SelectCategoryScreen extends StatefulWidget {
  ...
  static Route route(RouteSettings settings) {
    Map<String, dynamic> apiParameters = settings.arguments as Map<String, dynamic>;
...
class _SelectCategoryScreenState extends CloudState<SelectCategoryScreen> {
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion(
      ...
      child: SafeArea(
        child: Scaffold(
          appBar: UiUtils.buildAppBar(context, showBackButton: true, title: "adListing".translate(context), onBackPress: () {
              ...
              child: BlocBuilder<FetchCategoryCubit, FetchCategoryState>(builder: (context, state) {
                ...
                if (state is FetchCategorySuccess) {
                  return Column(
                    ...
                    children: [
                      Text("selectTheCategory".translate(context)).size(context.font.large).bold(weight: FontWeight.w600).color(context.color.textColorDark),
                      const SizedBox(
                        height: 18,
                      ),
                      GridView.builder(
...
class _SelectNestedCategoryState extends CloudState<SelectNestedCategory> {
  ...
  void getSubCategories() {
    if (widget.current.children!.isEmpty) {
      context.read<FetchSubCategoriesCubit>().fetchSubCategories(categoryId: widget.current.id!);
    }
  }
                                ...
                                return Row(
                                  children: [
                                    GestureDetector(
                                      ...
                                      child: Text(breadCrumbData[index].name!).firstUpperCaseWidget().color(
                                            isNotLast ? context.color.territoryColor : context.color.textColorDark,
                                          ),
                                    ),

                                    ///if it is not last
                                    if (isNotLast) const Text(" > ").color(context.color.territoryColor)
                                  ],
                                );
                              },
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return GestureDetector(
                                onTap: () {
                                  print(widget.current.children![index].children!.isEmpty);
                                  print(widget.current.children![index].subcategoriesCount);

                                  if (widget.current.children![index].children!.isEmpty && widget.current.children![index].subcategoriesCount == 0) {
                                    if (TouchManager.canProcessTouch()) {
                                      var currbalance = context.read<FetcBidCoinsCubit>().currbalance();
                                      if (currbalance < (widget.current.children![index].cost ?? 0)) {
                                        TouchManager.touchProcessed();
                                        ...
                                        Navigator.pushNamed(
                                          context,
                                          Routes.addItemDetails,
                                          arguments: <String, dynamic>{"breadCrumbItems": breadCrumbData..add(widget.current.children![index])},
                                        ).then((value) {
                                          screenStack--;
        ...
        itemBuilder: (context, index) {
          return Shimmer.fromColors(
            ...
            child: Container(
              ...
              height: 56,
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(5), border: Border.all(color: context.color.borderColor.darken(30))),
            ),
          );
        },
      ),
