...
class _ConfirmLocationScreenState extends CloudState<ConfirmLocationScreen> with WidgetsBindingObserver {
  ...
  preFillLocationWhileEdit() async {
    if (widget.isEdit!) {
      ...
      currentLocation = [itemModel.area, itemModel.city, itemModel.state, itemModel.country].where((part) => part != null && part.isNotEmpty).join(', ');
      formatedAddress = AddressComponent(area: itemModel.area, areaId: itemModel.areaId, city: itemModel.city, country: itemModel.country, state: itemModel.state);
      ...
      currentLocation = [HiveUtils.getCurrentAreaName(), HiveUtils.getCurrentCityName(), HiveUtils.getCurrentStateName(), HiveUtils.getCurrentCountryName()]
      ...
      if (currentLocation == "") {
        ...
        formatedAddress = AddressComponent(
            area: HiveUtils.getCurrentAreaName(), areaId: null, city: HiveUtils.getCurrentCityName(), country: HiveUtils.getCurrentCountryName(), state: HiveUtils.getCurrentStateName());
  ...
  getLocationFromLatitudeLongitude({LatLng? latLng}) async {
    try {
      Placemark? placeMark = (await placemarkFromCoordinates(latLng?.latitude ?? _cameraPosition!.target.latitude, latLng?.longitude ?? _cameraPosition!.target.longitude)).first;

      formatedAddress = AddressComponent(area: placeMark.subLocality, areaId: null, city: placeMark.locality, country: placeMark.country, state: placeMark.administrativeArea);
  ...
  void _showLocationServiceInstructions() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('pleaseEnableLocationServicesManually'.translate(context)),
        action: SnackBarAction(
          label: 'ok'.translate(context),
...
/*  Future<void> _getCurrentLocation() async {
    ...
    return PopScope(
      ...
      child: Scaffold(
          ...
          bottomNavigationBar: Padding(
            padding: const EdgeInsets.only(bottom: 12, left: 18.0, right: 18),
            child: UiUtils.buildButton(context, onPressed: () async {
              if (formatedAddress == null || (formatedAddress!.city == "" || formatedAddress!.city == null)) {
                HelperUtils.showSnackBarMessage(context, "cityRequired".translate(context));
                Future.delayed(Duration(seconds: 2), () {
                  dialogueBottomSheet(controller: cityTextController, title: "enterCity".translate(context), hintText: "city".translate(context), from: 1);
                });
                ...
                Future.delayed(Duration(seconds: 2), () {
                  dialogueBottomSheet(controller: countryTextController, title: "enterCountry".translate(context), hintText: "country".translate(context), from: 3);
                });
              } else {
                try {
              ...
              return;
            }, height: 48.rh(context), fontSize: context.font.large, autoWidth: false, radius: 8, width: double.maxFinite, buttonTitle: "postNow".translate(context)),
          ),
          body: bodyData()),
    );
  }
          ...
          ? Padding(
              ...
              child: Column(children: [
                Padding(
                  ...
                  ).bold(weight: FontWeight.bold).size(context.font.larger).centerAlign(),
                ),
                Padding(
                      ...
                      if (value != null) {
                        ...
                        if (mounted)
                          setState(() {
                            ...
                            print("latitude11****${location["latitude"]}*****${location["longitude"]}");
                            _cameraPosition = CameraPosition(
                              target: LatLng(latitude!, longitude!),
                              zoom: 14.4746,
                              bearing: 0,
                ...
                //               onCameraIdle: () async {
                //                 if (markerMove == false) {
                //                   if (LatLng(latitude!, longitude!) == LatLng(_cameraPosition!.target.latitude, _cameraPosition!.target.longitude)) {
                //                   } else {
                //                     getLocationFromLatitudeLongitude();
                //                   }
                //                 }
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(formatedAddress == null || (formatedAddress!.city == "" || formatedAddress!.city == null)
                                                ? "____"
                                                : (formatedAddress!.area != "" && formatedAddress!.area != null ? "${formatedAddress!.area}," : "") + formatedAddress!.city!)
                                            .size(context.font.large),
                                        const SizedBox(
                                          height: 4,
                                        ),
  ...
  void dialogueBottomSheet({required String title, required TextEditingController controller, required String hintText, required int from}) async {
    await UiUtils.showBlurredDialoge(
      context,
      dialoge: BlurredDialogBox(
        content: dialogueWidget(title, controller, hintText),
            ...
            children: [
              ...
              Padding(
                ...
                child: TextFormField(
                  maxLines: null,
                  textAlign: TextAlign.center,
                  style: TextStyle(fontWeight: FontWeight.bold, color: context.color.textDefaultColor.withOpacity(0.5)),
                  controller: controller,
                  cursorColor: context.color.territoryColor,
                  validator: (val) {
                    if (val == null || val.isEmpty) {
                  ...
                  decoration: InputDecoration(
                      ...
                      hintText: hintText,
                      hintStyle: TextStyle(fontWeight: FontWeight.bold, color: context.color.textDefaultColor.withOpacity(0.5)),
                      focusColor: context.color.territoryColor,
