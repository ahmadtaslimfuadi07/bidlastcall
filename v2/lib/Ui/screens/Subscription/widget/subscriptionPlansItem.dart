...
class SubscriptionPlansItem extends StatefulWidget {
  ...
  const SubscriptionPlansItem({super.key, required this.itemIndex, required this.index, required this.model});
...
/*  int calculateDiscountPercentage(int mainPrice, int discountPrice) {
          ...
          return BlocListener<GetPaymentIntentCubit, GetPaymentIntentState>(
            listener: (context, state) {
              ...
              if (state is GetPaymentIntentInSuccess) {
                ...
                if (_selectedGateway == "stripe") {
                  PaymentGateways.stripe(context, price: widget.model.finalPrice!.toDouble(), packageId: widget.model.id!, paymentIntent: state.paymentIntent);
                } else if (_selectedGateway == "paystack") {
                  PaymentGateways.paystack(context: context, price: widget.model.finalPrice!.toDouble(), packageId: widget.model.id!, orderId: state.paymentIntent["id"].toString());
            ...
            child: BlocListener<AssignFreePackageCubit, AssignFreePackageState>(
              ...
              child: Card(
                ...
                elevation: 0,
                margin: EdgeInsets.fromLTRB(14, (widget.index == widget.itemIndex) ? 40 : 70, 14, (widget.index == widget.itemIndex) ? 100 : 120),
                child: Column(
                  ...
                  children: [
                    ...
                    ClipPath(
                      ...
                      child: Container(
                        ...
                        child: UiUtils.imageType(widget.model.icon!, fit: BoxFit.contain),
                      ),
                    ),
                    SizedBox(height: 18.rh(context)),
                    widget.model.isActive! && widget.model.finalPrice! > 0 ? activeAdsData() : adsData(),
                    const Spacer(),
                    Text(widget.model.finalPrice! > 0 ? "${Constant.currencySymbol}${widget.model.finalPrice.toString()}" : "free".translate(context)).size(context.font.xxLarge).bold(),
                    if (widget.model.discount! > 0)
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text("${widget.model.discount}%\t${"OFF".translate(context)}").color(context.color.forthColor).bold(),
                    ...
                    if ((widget.index == widget.itemIndex))
                      ...
                      UiUtils.buildButton(context, onPressed: () {
                        UiUtils.checkUser(
                            onNotGuest: () {
                              if (!widget.model.isActive!) {
                                if (widget.model.finalPrice! > 0) {
                                  if (Platform.isIOS) {
                                    inAppPurchase.buy(widget.model.iosProductId!, widget.model.id!.toString());
                                  } else {
                          ...
                          border: widget.model.isActive! ? BorderSide(color: context.color.textDefaultColor.withOpacity(0.1)) : null,
                          buttonColor: widget.model.isActive! ? context.color.secondaryColor : context.color.territoryColor,
                          textColor: widget.model.isActive! ? context.color.textDefaultColor : context.color.secondaryColor,
                          buttonTitle: widget.model.isActive! ? "yourCurrentPlan".translate(context) : "purchaseThisPackage".translate(context),
    ...
    return Expanded(
      ...
      child: ListView(
        ...
        children: [
          ...
          SizedBox(height: 15),
          if (widget.model.type == "item_listing") checkmarkPoint(context, "${widget.model.limit.toString()}\t${"adsListing".translate(context)}"),
          if (widget.model.type == "advertisement") checkmarkPoint(context, "${widget.model.limit.toString()}\t${"featuredAdsListing".translate(context)}"),
          checkmarkPoint(context, "${widget.model.duration.toString()}\t${"days".translate(context)}"),
          if (widget.model.description != null && widget.model.description != "") checkmarkPoint(context, widget.model.description!),
        ],
      ),
    );
  }
  ...
  Widget activeAdsData() {
    return Expanded(
      ...
      child: ListView(
        ...
        shrinkWrap: true,
        children: [
          Text(widget.model.name!).copyWith(style: TextStyle(color: context.color.textDefaultColor, fontWeight: FontWeight.w600)).size(context.font.larger).centerAlign(),
          SizedBox(height: 15),
          if (widget.model.type == "item_listing")
            checkmarkPoint(context, "${widget.model.userPurchasedPackages![0].remainingItemLimit}/${widget.model.limit.toString()}\t${"adsListing".translate(context)}"),
          if (widget.model.type == "advertisement")
            checkmarkPoint(context, "${widget.model.userPurchasedPackages![0].remainingItemLimit}/${widget.model.limit.toString()}\t${"featuredAdsListing".translate(context)}"),
          checkmarkPoint(context, "${widget.model.userPurchasedPackages![0].remainingDays}/${widget.model.duration.toString()}\t${"days".translate(context)}"),
          if (widget.model.description != null && widget.model.description != "") checkmarkPoint(context, widget.model.description!),
        ],
      ),
    );
  ...
  SingleChildRenderObjectWidget bottomWidget() {
    if (widget.model.isActive! && widget.model.finalPrice! > 0 && widget.model.userPurchasedPackages != null && widget.model.userPurchasedPackages![0].endDate != null) {
      ...
      return Padding(
        ...
        child: Text("${"yourSubscriptionWillExpireOn".translate(context)} $formattedDate"),
      );
    } else {
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Text('selectPaymentMethod'.translate(context)).bold(weight: FontWeight.w600).size(context.font.larger).centerAlign(),
                  ),
                  ListView.builder(
                    shrinkWrap: true,
...
class PaymentMethodTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      trailing: isSelected ? Icon(Icons.check_circle, color: context.color.territoryColor) : Icon(Icons.radio_button_unchecked, color: context.color.textDefaultColor.withOpacity(0.5)),
