...
class PaymentGateways {
  ...
  static String generateReference(String email) {
    ...
    String reference = '${platform}_${email.split("@").first}_${DateTime.now().millisecondsSinceEpoch}';
    return reference;
  }

  static Future<void> stripe(BuildContext context, {required double price, required int packageId, required dynamic paymentIntent}) async {
    print("payment intent****${paymentIntent["client_secret"]}");
    String paymentIntentId = paymentIntent["id"].toString();
    String clientSecret = paymentIntent['payment_gateway_response']["client_secret"].toString();
  ...
  static Future<void> paystack({required BuildContext context, required price, required packageId, required orderId}) async {
    if (HiveUtils.getUserDetails().email != "" || HiveUtils.getUserDetails().email != null) {
      // Charge paystackCharge = Charge()
      //   ..amount = (price! * 100).toInt()
      //   ..email = HiveUtils.getUserDetails().email ?? ""
      //   ..currency = AppSettings.payStackCurrency
      //   ..reference = orderId; /*generateReference(HiveUtils.getUserDetails().email ?? "")*/
...
*/
      ...
      //     await payStackPlugin.checkout(context, logo: SizedBox(height: 50, width: 50, child: UiUtils.getSvg(AppIcons.splashLogo)), charge: paystackCharge, method: CheckoutMethod.card);
      ...
      //     Duration.zero,
      //     () {
      //       HelperUtils.showSnackBarMessage(context, UiUtils.getTranslatedLabel(context, "purchaseFailed"));
      //     },
      //   );
      // }
    } else {
      HelperUtils.showSnackBarMessage(context, "addEmailInYourProfile".translate(context));
    }
  }
...
/*  static Future<void> paystack(
  ...
  static void razorpay({required BuildContext context, required price, required orderId, required packageId}) {
    ...
    var options = {
      ...
      'description': '',
      'order_id': orderId,
      'prefill': {'contact': HiveUtils.getUserDetails().mobile ?? "", 'email': HiveUtils.getUserDetails().email ?? ""},
    ...
    if (AppSettings.razorpayKey != "") {
      ...
      HelperUtils.showSnackBarMessage(context, UiUtils.getTranslatedLabel(context, "setAPIkey"));
    }
  }
  ...
  static Future<void> _purchase(BuildContext context) async {
    try {
      Future.delayed(
        Duration.zero,
        () {
          HelperUtils.showSnackBarMessage(context, UiUtils.getTranslatedLabel(context, "success"), type: MessageType.success, messageDuration: 5);

          Navigator.of(context).popUntil((route) => route.isFirst);
        },
      );
    } catch (e) {
      HelperUtils.showSnackBarMessage(context, UiUtils.getTranslatedLabel(context, "purchaseFailed"), type: MessageType.error);
    }
  }
}
