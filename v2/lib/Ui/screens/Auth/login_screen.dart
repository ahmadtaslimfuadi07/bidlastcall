...
class LoginScreenState extends State<LoginScreen> {
  final TextEditingController emailMobileTextController = TextEditingController(text: Constant.isDemoModeOn ? Constant.demoMobileNumber : "");
  ...
  bool isObscure = true;
  late PhoneLoginPayload phoneLoginPayload = PhoneLoginPayload(emailMobileTextController.text, countryCode!);
  bool isBack = false;
    ...
    context.read<AuthenticationCubit>().listen((MLoginState state) {
      ...
      if (state is MVerificationPending) {
        if (mounted) {
          ...
          if (isMobileNumberField) {
            HelperUtils.showSnackBarMessage(context, "optsentsuccessflly".translate(context));
          }
        }
      }
      ...
      if (state is MFail) {
        ...
        if (isOtpSent && _otpController.text.isEmpty) {
          HelperUtils.showSnackBarMessage(context, "${"weSentCodeOnNumber".translate(context)}\t${emailMobileTextController.text}", type: MessageType.error);
        } else {
          if (state.error is FirebaseAuthException) {
            try {
              HelperUtils.showSnackBarMessage(context, (state.error as FirebaseAuthException).message!.toString());
            } catch (e) {}
          } else {
  ...
  void _onTapContinue() {
    if (isMobileNumberField) {
      // isOtpSent = true;
      phoneLoginPayload = PhoneLoginPayload(emailMobileTextController.text, countryCode!);
          ...
          child: PopScope(
            ...
            child: AnnotatedRegion(
              ...
              child: Scaffold(
                ...
                bottomNavigationBar: !isOtpSent && !sendMailClicked ? termAndPolicyTxt() : SizedBox.shrink(),
                  ...
                  listener: (context, state) {
                    if (state is LoginSuccess) {
                      ...
                      if (state.isProfileCompleted) {
                        print("HiveUtils.getCityName()******${HiveUtils.getCityName()}");
                        ...
                        Navigator.pushReplacementNamed(
                          ...
                          arguments: {
                            "from": "login",
                            "popToCurrent": false,
                            "type": isMobileNumberField ? AuthenticationType.phone : AuthenticationType.email,
                            "extraData": {
                        ...
                        if (state.type == AuthenticationType.email) {
                          ...
                          if (state.credential.user!.emailVerified) {
                            context.read<LoginCubit>().login(
                                phoneNumber: isMobileNumberField ? emailMobileTextController.text.trim() : null,
                                firebaseUserId: state.credential.user!.uid,
                                type: state.type.name,
                                credential: state.credential,
                                countryCode: isMobileNumberField ? "+${countryCode}" : null);
                          } else {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text("welcomeback".translate(context)).size(context.font.extraLarge).color(context.color.textDefaultColor),
              const SizedBox(
                height: 8,
              ),
              Text("login".translate(context)).size(context.font.large).color(
              ...
              CustomTextFormField(
                ...
                validator: isMobileNumberField ? CustomTextFieldValidator.phoneNumber : CustomTextFieldValidator.email,
                        ...
                        child: Align(
                            ...
                            child: GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: Center(child: Text("+$countryCode").size(context.font.large).centerAlign()),
                              ),
                            )),
                      )
                    : null,
              ...
              UiUtils.buildButton(context,
                  onPressed: sendVerificationCode, buttonTitle: "continue".translate(context), radius: 10, disabled: numberOrEmail.isEmpty, disabledColor: const Color.fromARGB(255, 104, 102, 106)),
              const SizedBox(
                height: 68,
              ),
              Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text("dontHaveAcc".translate(context)).color(context.color.textColorDark.brighten(50)),
                      const SizedBox(
                        width: 12,
                      ),
                      GestureDetector(
                        ...
                        child: Text("signUp".translate(context)).underline().color(context.color.territoryColor),
                      )
                    ],
                  ),
                  const SizedBox(
                  ...
                  Text("orSignInWith".translate(context)).color(context.color.textDefaultColor),
                  const SizedBox(
                    height: 24,
                  ),
                  UiUtils.buildButton(context,
                      ...
                      border: context.watch<AppThemeCubit>().state.appTheme != AppTheme.dark ? BorderSide(color: context.color.textDefaultColor.withOpacity(0.5)) : null,
  ...
  Widget termAndPolicyTxt() {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text("bySigningUpLoggingIn".translate(context)).centerAlign().size(context.font.small).color(context.color.textLightColor.withOpacity(0.8)),
          const SizedBox(
            height: 3,
          ),
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            InkWell(
                child: Text("termsOfService".translate(context)).underline().color(context.color.territoryColor).size(context.font.small),
                onTap: () => Navigator.pushNamed(context, Routes.profileSettings, arguments: {'title': "termsConditions".translate(context), 'param': Api.termsAndConditions})),
            /*CustomTextButton(
            ...
            Text("andTxt".translate(context)).size(context.font.small).color(context.color.textLightColor.withOpacity(0.8)),
            const SizedBox(
              width: 5.0,
            ),
            InkWell(
                child: Text("privacyPolicy".translate(context)).underline().color(context.color.territoryColor).size(context.font.small),
                onTap: () => Navigator.pushNamed(context, Routes.profileSettings, arguments: {'title': "privacyPolicy".translate(context), 'param': Api.privacyPolicy})),
            /*CustomTextButton(
                text:
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text("signInWithMob".translate(context)).size(context.font.extraLarge),
          const SizedBox(
            height: 8,
          ),
          Row(
            children: [
              Text("+${phoneLoginPayload.countryCode}\t${phoneLoginPayload.phoneNumber}").size(context.font.large),
              const SizedBox(
                width: 5,
              ),
              InkWell(child: Text("change".translate(context)).underline().color(context.color.territoryColor).size(context.font.large), onTap: () => Navigator.pushNamed(context, Routes.login)),
            ],
          ),
          const SizedBox(
            height: 24,
          ...
          Align(
            ...
            child: MaterialButton(
              ...
              child: Text("resendOTP".translate(context)).color(context.color.textColorDark.withOpacity(0.7)),
            ),
          ),
          const SizedBox(
            height: 19,
        ...
        children: [
          ...
          Align(
            ...
            child: MaterialButton(
              ...
              child: Text("${"forgotPassword".translate(context)}?").color(context.color.textLightColor).size(context.font.normal),
            ),
          ),
          const SizedBox(
            height: 19,
          ...
          UiUtils.buildButton(
            ...
            onPressed: () {
              context
                  ...
                  .setData(payload: EmailLoginPayload(email: emailMobileTextController.text, password: _passwordController.text, type: EmailLoginType.login), type: AuthenticationType.email);
              context.read<AuthenticationCubit>().authenticate();
            },
            buttonTitle: "signIn".translate(context),
            radius: 8,
