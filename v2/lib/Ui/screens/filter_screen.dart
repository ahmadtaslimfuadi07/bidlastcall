...
class FilterScreenState extends State<FilterScreen> {
  ...
  bool checkFilterValSet() {
    if (postedOn != Constant.postedSince[0].value || minController.text.trim().isNotEmpty || maxController.text.trim().isNotEmpty || selectedCategory != defaultCategory) {
      return true;
    }
...
/*    FocusManager.instance.primaryFocus?.unfocus();
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: BottomAppBar(
          ...
          child: UiUtils.buildButton(context, outerPadding: const EdgeInsets.all(12), height: 50.rh(context), onPressed: () {
            print("field data******${AbstractField.fieldsData.length}");

            Map<String, dynamic> customFields = convertToCustomFields(AbstractField.fieldsData);
      ...
      "",*/
            ...
            widget.update(ItemFilterModel(
                maxPrice: maxController.text,
                minPrice: minController.text,
                categoryId: widget.from == "search" ? ((selectedCategory is String) ? selectedCategory : selectedCategory?.id) ?? "" : '',
                postedSince: postedOn,
                city: city,
                areaId: areaId,
                state: _state,
        ...
        body: SingleChildScrollView(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                Text('locationLbl'.translate(context)).bold(weight: FontWeight.w600).color(context.color.textDefaultColor),
                ...
                Text('budgetLbl'.translate(context)).bold(weight: FontWeight.w600).color(context.color.textDefaultColor),
                const SizedBox(height: 15),
                budgetOption(),
                const SizedBox(height: 15),
                Text('postedSinceLbl'.translate(context)).bold(weight: FontWeight.w600).color(context.color.textDefaultColor),
                const SizedBox(height: 5),
    ...
    return BlocConsumer<FetchCustomFieldsCubit, FetchCustomFieldState>(
      listener: (context, state) {
        if (state is FetchCustomFieldSuccess) {
          moreDetailDynamicFields = context.read<FetchCustomFieldsCubit>().getFields().where((field) => field.type != "fileinput").map((field) {
            print("field****${field.value}");
            Map<String, dynamic> fieldData = field.toMap();

            // Prefill value from Constant.itemFilter!.customFields
            if (Constant.itemFilter != null && Constant.itemFilter!.customFields != null) {
              String customFieldKey = 'custom_fields[${fieldData['id']}]';
              print("customFieldKey****$customFieldKey");
              if (Constant.itemFilter!.customFields!.containsKey(customFieldKey)) {
                fieldData['value'] = Constant.itemFilter!.customFields![customFieldKey];
  ...
  Widget categoryModule() {
    if (widget.from == "search") {
      return Column(
        ...
        children: [
          const SizedBox(height: 15),
          Text("category".translate(context)).bold(weight: FontWeight.w600).color(context.color.textDefaultColor),
          const SizedBox(height: 5),
        ...
        child: Container(
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                UiUtils.getSvg(AppIcons.locationIcon, color: context.color.textDefaultColor),
                Expanded(
                  child: Padding(
                    ...
                    child: (city != "" && city != null)
                        ...
                        : Text("allCities".translate(context)).color(context.color.textDefaultColor.withOpacity(0.5)),
                  ),
                ),
              ],
            ),
        ...
        return categoryList.last.children!.isNotEmpty
            ? Column(
                ...
                children: [
                  ...
                  Text("category".translate(context)).bold(weight: FontWeight.w600).color(context.color.textDefaultColor),
                  const SizedBox(height: 5),
                  subCategoryWidget(context, categoryList[index], "All in ${categoryList[index].name}", context.color.textDefaultColor.withOpacity(0.5)),
                ],
              )
            : Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  Text("category".translate(context)).bold(weight: FontWeight.w600).color(context.color.textDefaultColor),
                  const SizedBox(height: 5),
                  subCategoryWidget(context, categoryList[index], categoryList[index].name!, context.color.textDefaultColor),
                ],
              );
      }));
    } else {
  ...
  Widget subCategoryWidget(BuildContext context, CategoryModel model, String name, Color color) {
    return InkWell(
      onTap: () {
        if (model.children!.isNotEmpty) {
          Navigator.pushNamed(context, Routes.subCategoryFilterScreen, arguments: {"addModel": getCategoryModel, "model": model.children});
        }
      },
      child: Padding(
        padding: const EdgeInsets.only(top: 10.0),
        child: Container(
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                UiUtils.getImage(model.url!, height: 20, width: 20, fit: BoxFit.contain),
                Padding(padding: const EdgeInsetsDirectional.only(start: 15.0), child: Text(name).color(color)),
                Spacer(),
                Padding(
  ...
  Widget categoryWidget(BuildContext context) {
    return InkWell(
      onTap: () {
        Navigator.pushNamed(context, Routes.categoryFilterScreen, arguments: {"addModel": getCategoryModel}).then((value) {
          if (value != null) {
            ...
            child: Row(
              children: [
                ...
                Padding(
                  ...
                  child: categoryList.isNotEmpty ? Text("${categoryList[0].name}") : Text("allCategories".translate(context)).color(context.color.textDefaultColor.withOpacity(0.5)),
                ),
                Spacer(),
                Padding(
                  padding: EdgeInsetsDirectional.only(end: 14.0),
  ...
  Widget minMaxTFF(String minMax) {
    return Container(
        ...
        decoration: BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(10)), color: Theme.of(context).colorScheme.secondaryColor),
        child: TextFormField(
            controller: (minMax == "minLbl".translate(context)) ? minController : maxController,
            onChanged: ((value) {
            ...
            decoration: InputDecoration(
                ...
                enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: context.color.borderColor.darken(30))),
                labelStyle: TextStyle(color: context.color.textDefaultColor.withOpacity(0.5)),
                hintText: "00",
                label: Text(
                  minMax,
                ),
  ...
  Widget postedSinceOption(BuildContext context) {
    ...
    return InkWell(
      onTap: () {
        Navigator.pushNamed(context, Routes.postedSinceFilterScreen, arguments: {"list": Constant.postedSince, "postedSince": postedOn, "update": postedSinceUpdate}).then((value) {});
      },
      child: Padding(
        padding: const EdgeInsets.only(top: 10.0),
        child: Container(
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                Padding(
                  padding: const EdgeInsetsDirectional.only(start: 15.0),
                  child: Text(Constant.postedSince[index].status).color(context.color.textDefaultColor.withOpacity(0.5)),
                ),
                Spacer(),
                Padding(
                  padding: EdgeInsetsDirectional.only(end: 14.0),
  ...
  void onClickPosted(String val) {
    if (val == Constant.postedSince[0].value && searchbody.containsKey(Api.postedSince)) {
