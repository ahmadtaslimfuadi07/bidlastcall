...
import 'package:eClassify/Ui/screens/bidCoins/bid_coins_screen.dart';
...
class MainActivity extends StatefulWidget {
  ...
  static Route route(RouteSettings routeSettings) {
    ...
    return BlurredRouter(builder: (_) => MainActivity(from: arguments['from'] as String));
  }
}
...
/*  void handleDeepLink(Uri uri) {
            ...
            UiUtils.showBlurredDialoge(context,
                dialoge: BlurredDialogBox(
                    ...
                    content: Column(
                      ...
                      children: [
                        Text("$current>$remote"),
                        Text("newVersionAvailableForce".translate(context), textAlign: TextAlign.center),
                      ],
                    )));
          } else {
  ...
  void locationSetCheck() {
    if (HiveUtils.isShowChooseLocationDialoge() && !HiveUtils.isLocationFilled()) {
      Future.delayed(
        Duration.zero,
        () {
          UiUtils.showBlurredDialoge(
            ...
            dialoge: BlurredDialogBox(
              ...
              onAccept: () async {
                ...
                Navigator.pushNamed(context, Routes.completeProfile, arguments: {"from": "chooseLocation", "navigateToHome": true});
              },
            ),
          );
        },
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion(
      ...
      child: PopScope(
        ...
        onPopInvoked: (didPop) {
          if (currtab != 0) {
            pageCntrlr.animateToPage(0, duration: const Duration(milliseconds: 400), curve: Curves.easeInOut);
            setState(() {
              currtab = 0;
              isBack = false;
            });
  ...
  void onItemTapped(int index) {
    ...
    if (index == currtab) {
      ...
      if (controllerList[index].hasClients) {
        controllerList[index].animateTo(0, duration: const Duration(milliseconds: 200), curve: Curves.bounceOut);
      }
    }
    ...
    return BottomAppBar(
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.spaceAround, children: <Widget>[
          ...
          buildBottomNavigationbarItem(1, AppIcons.chatNav, AppIcons.chatNavActive, "Bid Coins"),

          BlocListener<FetcBidCoinsCubit, FetchBidCoinsCubitState>(
              listener: (context, state) {
                if (currtab == 4) {
                  ...
                  if (state is FetchBidCoinsSuccess) {
                    (state.bidCoinsHistoryModel.currbalance ?? 0) > 0
                        ? UiUtils.checkUser(
                            onNotGuest: () {
                              Navigator.pushNamed(context, Routes.selectCategoryScreen, arguments: <String, dynamic>{});
                            },
                            context: context)
                        : UiUtils.noCoinDialog(context, () {
                            currtab = 1;
                            pageCntrlr.jumpToPage(currtab);
                            setState(() {});
                          });
          ...
          //       ),
          //     )),
          buildBottomNavigationbarItem(2, AppIcons.myAdsNav, AppIcons.myAdsNavActive, "Bid Trans".translate(context)),
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Column(
            ...
            children: <Widget>[
              if (currtab == index) ...{
                UiUtils.getSvg(activeSvg, height: 20),
              } else ...{
                UiUtils.getSvg(svgImage, color: context.color.textLightColor.darken(30)),
              },
              Text(
                title,
                textAlign: TextAlign.center,
              ).color(currtab == index ? context.color.textDefaultColor : context.color.textLightColor.darken(30)),
            ],
          ),
        ),
      ),
