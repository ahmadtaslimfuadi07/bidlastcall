        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              ).size(context.font.larger).color(context.color.textDefaultColor.withOpacity(0.65)).centerAlign(),
              const SizedBox(height: 58),
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 12),
                child: UiUtils.buildButton(context, showElevation: false, buttonColor: context.color.territoryColor, textColor: context.color.secondaryColor, onPressed: () {
                  // Check location permission when the button is pressed
                  _getCurrentLocation();
                }, radius: 8, height: 46, buttonTitle: "allowLocationAccess".translate(context)),
              ),
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 12),
                child: UiUtils.buildButton(context,
                    showElevation: false, buttonColor: context.color.backgroundColor, border: BorderSide(color: context.color.territoryColor), textColor: context.color.territoryColor, onPressed: () {
                  Navigator.pushNamed(context, Routes.countriesScreen, arguments: {"from": "location"});
                }, radius: 8, height: 46, buttonTitle: "enterManually".translate(context)),
              ),
            ],
          ),
        ),
