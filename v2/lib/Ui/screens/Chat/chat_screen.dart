...
class _ChatScreenState extends State<ChatScreen> with SingleTickerProviderStateMixin {
  ...
  late final ScrollController _pageScrollController = ScrollController()
    ..addListener(
      () {
        if (_pageScrollController.offset >= _pageScrollController.position.maxScrollExtent) {
        ...
        child: Scaffold(
                ...
                children: [
                  if (messageAttachment != null) ...[
                    if (supportedImageTypes.contains(attachmentMIME)) ...[
                      Container(
                        decoration: BoxDecoration(color: context.color.secondaryColor, border: Border.all(color: context.color.borderColor, width: 1.5)),
                        child: Row(
                          children: [
                            Padding(
                  ...
                  BottomAppBar(
                    ...
                    child: Directionality(
                      ...
                      child: widget.status == "review" || widget.status == "rejected" || widget.status == "sold out" || widget.status == "inactive"
                          ? Container(
                              height: 40,
                              width: double.maxFinite,
                              color: context.color.secondaryColor,
                              alignment: Alignment.center,
                              child: Text("${"thisItemIs".translate(context)} ${widget.status}").size(context.font.large))
                          : Column(
                              children: [
                                BlocProvider(
                                    create: (context) => UnblockUserCubit(),
                                    child: Builder(builder: (context) {
                                      bool isBlocked = context.read<BlockedUsersListCubit>().isUserBlocked(int.parse(widget.userId));
                                      return BlocConsumer<BlockedUsersListCubit, BlockedUsersListState>(listener: (context, state) {
                                        if (state is BlockedUsersListSuccess) {
                                          isBlocked = context.read<BlockedUsersListCubit>().isUserBlocked(int.parse(widget.userId));
                                        }
                                            ...
                                            ? BlocListener<UnblockUserCubit, UnblockUserState>(
                                                listener: (context, unblockState) {
                                                  if (unblockState is UnblockUserSuccess) {
                                                    ...
                                                    context.read<BlockedUsersListCubit>().unblockUser(int.parse(widget.userId));
                                                ...
                                                child: InkWell(
                                                  child: Text("youBlockedThisContact".translate(context)).color(context.color.textColorDark.withOpacity(0.7)),
                                                  onTap: () async {
                                                    var unBlock = await UiUtils.showBlurredDialoge(
                                                      ...
                                                      dialoge: BlurredDialogBox(
                                                        ...
                                                        content: Text(
                                                          "${"unBlockLbl".translate(context)}\t${widget.userName}\t${"toSendMessage".translate(context)}".translate(context),
                                                        ),
                                                      ),
                                                    );
                                                    if (unBlock == true) {
                                          ...
                                          ChatMessageHandler.add(
                                            BlocProvider(
                                              ...
                                              child: ChatMessage(
                                                ...
                                                updatedAt: DateTime.now().toString(),
                                                audio: "",
                                                file: messageAttachment != null ? messageAttachment?.path : "",
                                                itemOfferId: widget.itemOfferId,
                                              ),
                                            ),
                                          );
          ...
          appBar: AppBar(
            ...
            bottom: PreferredSize(
              preferredSize: Size.fromHeight(isNotificationPermissionGranted ? 63 : 98),
              child: Column(
                          ...
                          child: GestureDetector(
                            onTap: () async {
                              try {
                                ...
                                DataOutput<ItemModel> dataOutput = await ItemRepository().fetchItemFromItemId(int.parse(widget.itemId));
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Padding(
                                  ...
                                  child: Text(
                                    Constant.currencySymbol.toString() + widget.itemPrice.toString(), // Replace with your item price
                                  ).color(context.color.textDefaultColor).size(context.font.large).bold(),
                                ),
                              ],
                            ),
                          ),
            ...
            actions: [
              MultiBlocProvider(
                ...
                child: Builder(builder: (context) {
                  bool isBlocked = context.read<BlockedUsersListCubit>().isUserBlocked(int.parse(widget.userId));
                  return BlocConsumer<BlockedUsersListCubit, BlockedUsersListState>(
                        ...
                        listener: (context, blockState) {
                          if (blockState is BlockUserSuccess) {
                            ...
                            context.read<BlockedUsersListCubit>().addBlockedUser(
                                  BlockedUserModel(id: int.parse(widget.userId), name: widget.userName, profile: widget.profilePicture
                            ...
                            HelperUtils.showSnackBarMessage(context, blockState.error.toString());
                          }
                        },
                              ...
                              child: PopupMenuButton(
                                ...
                                itemBuilder: (context) => [
                                  if (!isBlocked)
                                    PopupMenuItem(
                                      onTap: () async {
                                        var block = await UiUtils.showBlurredDialoge(
                                          context,
                                          dialoge: BlurredDialogBox(
                                            acceptButtonName: "blockLbl".translate(context),
                                            title: "${"blockLbl".translate(context)}\t${widget.userName}?",
                                            content: Text(
                                      ...
                                      child: Text("blockLbl".translate(context)).color(context.color.textColorDark),
                                    )
                                  else
                                    PopupMenuItem(
                                      onTap: () async {
                                      ...
                                      child: Text("unBlockLbl".translate(context)).color(context.color.textColorDark),
                                    ),
                                ],
                              ),
                            ),
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(
                    ...
                    child: Text(widget.userName).color(context.color.textColorDark).size(context.font.normal),
                  ),
                ],
              ),
            ),
                  ...
                  child: BlocConsumer<LoadChatMessagesCubit, LoadChatMessagesState>(
                    listener: (context, state) {
                      ...
                      if (state is LoadChatMessagesSuccess) {
                        print("LoadChatMessagesSuccess with ${state.messages.length} messages");
...
/*                          BlocBuilder<ChatMessageHandlerCubit, List<Widget>>(
                          ...
                          StreamBuilder<List<Widget>>(
                              ...
                              builder: (context, AsyncSnapshot<List<Widget>> snapshot) {
                                print("StreamBuilder triggered: ${snapshot.connectionState}****${state}");
                                Widget? loadingMoreWidget;
                                if (state is LoadChatMessagesSuccess) {
                                ...
                                if (snapshot.connectionState == ConnectionState.active || snapshot.connectionState == ConnectionState.done) {
                                  print("Snapshot has data: ${snapshot.data!.length} items");
                                  ...
                                  if (snapshot.hasData) {
                                    print("Displaying ${snapshot.data!.length} messages");
                                        ...
                                        Expanded(
                                          child: ListView.builder(
                                            ...
                                            itemBuilder: (context, index) {
                                              ...
                                              return Column(
                                                mainAxisSize: MainAxisSize.min,
                                                children: [if (index == ((snapshot.data as List).length - 1)) offerWidget(), if (index == snapshot.data!.length - 1) offerWidget(), chat],
                                              );
                                            },
                                          ),
                                        ),
      ...
      return Align(
        ...
        child: Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.only(topRight: Radius.circular(0), topLeft: Radius.circular(8), bottomRight: Radius.circular(8), bottomLeft: Radius.circular(8))),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("yourOffer".translate(context)).color(context.color.textDefaultColor.withOpacity(0.5)),

                /*  Text("yourOffer".translate(context))
                  .color(context.color.textDefaultColor.withOpacity(0.5)),*/
                Text(Constant.currencySymbol + widget.itemOfferPrice.toString()).bold().size(context.font.larger).color(context.color.textDefaultColor)
              ],
            )),
      );
    } else {
      return Align(
        ...
        child: Container(
            ...
            decoration: BoxDecoration(
                border: Border.all(color: context.color.territoryColor.withOpacity(0.3)),
                color: context.color.territoryColor.withOpacity(0.17),
                borderRadius: BorderRadius.only(topRight: Radius.circular(8), topLeft: Radius.circular(0), bottomRight: Radius.circular(8), bottomLeft: Radius.circular(8))),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("offerLbl".translate(context)).color(context.color.textDefaultColor.withOpacity(0.5)),
                Text(Constant.currencySymbol + widget.itemOfferPrice.toString()).bold().size(context.font.larger).color(context.color.textDefaultColor)
              ],
            )),
      );
