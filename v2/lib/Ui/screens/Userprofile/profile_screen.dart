...
class _ProfileScreenState extends State<ProfileScreen> with AutomaticKeepAliveClientMixin<ProfileScreen> {
  ...
  Widget setIconButtons({
    ...
    return Container(
      ...
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(10), border: Border.all(color: context.color.textDefaultColor.withOpacity(0.1))),
      child: InkWell(
          onTap: onTap,
              ...
              child: Column(children: <Widget>[
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              SizedBox(
                                ...
                                child: Text(username).color(context.color.textColorDark).size(context.font.large).bold(weight: FontWeight.w700).setMaxLines(lines: 1),
                              ),
                              SizedBox(
                                width: context.screenWidth * 0.35,
                                child: Text(email).color(context.color.textColorDark).size(context.font.small).setMaxLines(lines: 1),
                              ),
                            ],
                          ),
                        ),
                ...
                Column(
                  ...
                  children: [
                    ...
                    customTile(
                      context,
                      title: "Bid History".translate(context),
                      svgImagePath: AppIcons.transaction,
                      onTap: () {
                        UiUtils.checkUser(
                            onNotGuest: () {
                              Navigator.pushNamed(context, Routes.bidTransactionHistory);
                            },
                            context: context);
                      },
                    ),
                    customTile(
                      context,
                      title: "Sales History".translate(context),
                      svgImagePath: AppIcons.transaction,
                      onTap: () {
                        UiUtils.checkUser(
                            onNotGuest: () {
                    ...
                    customTile(
                      ...
                      onTap: () {
                        Navigator.pushNamed(context, Routes.profileSettings, arguments: {'title': "termsConditions".translate(context), 'param': Api.termsAndConditions});
                      },
                    ),
                    customTile(
                      context,
                      title: "T&C Seller".translate(context),
                      svgImagePath: AppIcons.terms,
                      onTap: () {
                        Navigator.pushNamed(context, Routes.profileSettings, arguments: {'title': "T&C Seller".translate(context), 'param': Api.termsAndConditionsSeller});
                      },
                    ),
                      ...
                      customTile(
                        ...
                        onTap: () {
                          if (Constant.isDemoModeOn) {
                            if (HiveUtils.getUserDetails().mobile != null) if (Constant.demoMobileNumber == (HiveUtils.getUserDetails().mobile!.replaceFirst("+${HiveUtils.getCountryCode()}", ""))) {
        ...
        child: Row(
          children: [
            Container(
              ...
              child: FittedBox(fit: BoxFit.none, child: isUpdateAvailable == false ? const Icon(Icons.done) : UiUtils.getSvg(svgImagePath, color: context.color.territoryColor)),
            ),
            SizedBox(
              width: 25.rw(context),
            ),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(isUpdateAvailable == false ? "uptoDate".translate(context) : title).bold(weight: FontWeight.w700).color(context.color.textColorDark),
                if (isUpdateAvailable) Text("v$newVersion").bold(weight: FontWeight.w300).color(context.color.textColorDark).size(context.font.small).italic()
              ],
            ),
            if (isUpdateAvailable) ...[
              const Spacer(),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: FittedBox(fit: BoxFit.none, child: UiUtils.getSvg(svgImagePath, height: 24, width: 24, color: context.color.territoryColor)),
                ),
                SizedBox(
                  width: 25.rw(context),
                ),
                Expanded(
                  flex: 3,
                  child: Text(title).bold(weight: FontWeight.w700).color(context.color.textColorDark),
                ),
                const Spacer(),
                if (isSwitchBox != true)
                  Container(
  ...
  deleteConfirmWidget() {
    UiUtils.showBlurredDialoge(
      ...
      dialoge: BlurredDialogBox(
        title: (_auth.currentUser != null) ? "deleteProfileMessageTitle".translate(context) : "deleteAlertTitle".translate(context),
        content: _auth.currentUser != null
            ...
            : Text("deleteRelogin".translate(context), textAlign: TextAlign.center),
        cancelButtonName: (_auth.currentUser != null) ? 'no'.translate(context) : 'cancelLbl'.translate(context),
        acceptButtonName: (_auth.currentUser != null) ? "deleteBtnLbl".translate(context) : 'logout'.translate(context),
