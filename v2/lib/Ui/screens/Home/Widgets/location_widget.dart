    ...
    return FittedBox(
      ...
      child: Row(
        ...
        children: [
          GestureDetector(
            onTap: () async {
              Navigator.pushNamed(context, Routes.countriesScreen, arguments: {"from": "home"});
              /* var result = await showModalBottomSheet(
                isScrollControlled: true,
                shape: const RoundedRectangleBorder(
                    borderRadius: BorderRadius.only(
          ...
          ValueListenableBuilder(
              ...
              builder: (context, value, child) {
                ...
                return Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("locationLbl".translate(context)).color(context.color.textColorDark).size(
                          context.font.small,
                        ),
                    Text(
                      HiveUtils.getCityName() == null
                          ...
                          : (HiveUtils.getAreaName() ?? "") +
                              ((HiveUtils.getAreaName() == null || HiveUtils.getAreaName() == "" ? "" : ",") +
                                  (HiveUtils.getCityName() ?? "") +
                                  (HiveUtils.getStateName() == null || HiveUtils.getStateName() == "" ? "" : ",") +
                                  (HiveUtils.getStateName() ?? "") +
                                  (HiveUtils.getCountryName() == null || HiveUtils.getCountryName() == "" ? "" : ",") +
                                  (HiveUtils.getCountryName() ?? "")) +
                              "",
                      maxLines: 1,
                      softWrap: true,
                      overflow: TextOverflow.ellipsis,
                    ).color(context.color.textColorDark).size(context.font.small).bold(weight: FontWeight.w600),
                  ],
                );
              }),
        ],
