...
class LanguagesListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (context.watch<FetchSystemSettingsCubit>().getSetting(SystemSetting.language) == null) {
      return Scaffold(
        backgroundColor: context.color.primaryColor,
        appBar: UiUtils.buildAppBar(context, showBackButton: true, title: "chooseLanguage".translate(context)),
        body: Center(child: UiUtils.progress()),
      );
    }

    List setting = context.watch<FetchSystemSettingsCubit>().getSetting(SystemSetting.language) as List;
      ...
      body: BlocListener<FetchLanguageCubit, FetchLanguageState>(
        ...
        child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              Color color = (language as LanguageLoader).language['code'] == setting[index]['code'] ? context.color.territoryColor : context.color.textLightColor.withOpacity(0.03);
              ...
              return Padding(
                ...
                child: Container(
                  ...
                  child: ListTile(
                    onTap: () {
                      context.read<FetchLanguageCubit>().getLanguage(setting[index]['code']);
                    },
                    leading: Container(
                    ...
                    title: Column(
                      ...
                      children: [
                        Text(setting[index]['name']).color((language).language['code'] == setting[index]['code'] ? context.color.buttonColor : context.color.textColorDark).bold(),
                        Text(setting[index]['name_in_english'])
                            .color((language).language['code'] == setting[index]['code'] ? context.color.buttonColor.withOpacity(0.7) : context.color.textColorDark.withOpacity(0.6))
