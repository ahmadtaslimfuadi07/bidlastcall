        ...
        if (state is SliderFetchSuccess && state.sliderlist.isNotEmpty) {
          ...
          return SizedBox(
            ...
            child: PageView.builder(
              ...
              scrollDirection: Axis.horizontal,
              onPageChanged: (index) {
                _bannerIndex.value = index; // Update bannerIndex when page changes manually
              },
              itemBuilder: (context, index) {
                return InkWell(
                  onTap: () async {
                    if (state.sliderlist[index].thirdPartyLink != "") {
                      await urllauncher.launchUrl(Uri.parse(state.sliderlist[index].thirdPartyLink!), mode: LaunchMode.externalApplication);
                    } else if (state.sliderlist[index].modelType!.contains("Category")) {
                      if (state.sliderlist[index].model!.subCategoriesCount! > 0) {
                        Navigator.pushNamed(context, Routes.subCategoryScreen, arguments: {
                          ...
                          "categoryIds": [state.sliderlist[index].model!.parentCategoryId.toString(), state.sliderlist[index].modelId.toString()]
                        });
                      } else {
                        Navigator.pushNamed(context, Routes.itemsList, arguments: {
                          'catID': state.sliderlist[index].modelId.toString(),
                          'catName': state.sliderlist[index].model!.name,
                          "categoryIds": [state.sliderlist[index].modelId.toString()]
                        });
                      }
                    } else {
                      try {
                        ...
                        DataOutput<ItemModel> dataOutput = await fetch.fetchItemFromItemId(state.sliderlist[index].modelId!);
                  ...
                  child: Container(
                    ...
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(10),
                      child: UiUtils.getImage(state.sliderlist[index].image ?? "", fit: BoxFit.fill),
                    ),
                  ),
                );
              },
