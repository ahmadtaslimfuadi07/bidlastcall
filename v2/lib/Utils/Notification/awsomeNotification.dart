...
class LocalAwsomeNotification {
  ...
  void init(BuildContext context) {
    ...
    notification.initialize(
        ...
        [
          NotificationChannel(
              channelKey: Constant.notificationChannel, channelName: 'Basic notifications', channelDescription: 'Notification channel', importance: NotificationImportance.Max, ledColor: Colors.grey),
          NotificationChannel(
              channelKey: "Chat Notification", channelName: 'Chat Notifications', channelDescription: 'Chat Notifications', importance: NotificationImportance.Max, ledColor: Colors.grey)
        ],
        channelGroups: [],
        debug: true);
    listenTap(context);
  ...
  createNotification({
    ...
    try {
      ...
      bool hasImage = notificationData.data["image"] != null || notificationData.data["image"] != "";

      if (isChat) {
        int chatId = int.parse(notificationData.data['sender_id']) + int.parse(notificationData.data['item_id']);
  ...
  static Future<void> onActionReceivedMethod(ReceivedAction receivedAction) async {
    ...
    if (payload?['type'] == "chat") {
      ...
      Future.delayed(
        Duration.zero,
        () {
          Navigator.push(Constant.navigatorKey.currentContext!, MaterialPageRoute(
            builder: (context) {
      ...
      Future.delayed(Duration.zero, () {
        Future.delayed(Duration.zero, () {
          Navigator.popUntil(Constant.navigatorKey.currentContext!, (route) => route.isFirst);
      ...
      DataOutput<ItemModel> item = await ItemRepository().fetchItemFromItemId(int.parse(id));

      Future.delayed(
        Duration.zero,
        () {
          HelperUtils.goToNextPage(Routes.adDetailsScreen, Constant.navigatorKey.currentContext!, false, args: {
      ...
      Future.delayed(Duration.zero, () {
        HelperUtils.goToNextPage(Routes.notificationPage, Constant.navigatorKey.currentContext!, false);
      });
