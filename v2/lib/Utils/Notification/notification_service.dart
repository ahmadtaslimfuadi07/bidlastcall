...
class NotificationService {
  ...
  static handleNotification(RemoteMessage? message, [BuildContext? context]) {
    ...
    if (notificationType == "chat") {
      ...
      if (userType == "Buyer") {
        (context as BuildContext).read<GetBuyerChatListCubit>().addNewChat(ChatedUser(
              ...
              /* sellerId: senderId,*/
              updatedAt: date,
              item: Item(id: int.parse(itemId), price: int.parse(itemPrice), name: itemName, image: itemImage),
              /*seller: Seller(name: username, profile: userProfile),*/
              buyerId: int.parse(senderId),
              buyer: Buyer(name: username, profile: userProfile, id: int.parse(senderId)),
            ));
      } else {
        (context as BuildContext).read<GetBuyerChatListCubit>().addNewChat(ChatedUser(
              itemId: int.parse(itemId),
              ...
              updatedAt: date,
              item: Item(id: int.parse(itemId), price: int.parse(itemPrice), name: itemName, image: itemImage),
              seller: Seller(name: username, profile: userProfile, id: int.parse(senderId)),
            ));
      }

      ///Checking if this is user we are chatiing with
      print('currentlyChatingWith***$currentlyChatingWith****$currentlyChatItemId******${senderId == currentlyChatingWith && itemId == currentlyChatItemId}');
  ...
  static void onTapNotificationHandler(context) {
    onMessageOpen = FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
