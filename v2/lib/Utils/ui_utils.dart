...
class UiUtils {
  ...
  Duration differenceDate(String dateTime) {
  ...
  static checkUser({required Function() onNotGuest, required BuildContext context}) {
        ...
        return Container(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text("loginIsRequiredForAccessingThisFeatures".translate(context)).size(context.font.larger),
                const SizedBox(
                  height: 5,
                ),
                Text("tapOnLoginToAuthorize".translate(context)).size(context.font.small),
                const SizedBox(
                  height: 10,
                ),
                MaterialButton(
  ...
  static Widget getSvgImage(String url, {double? width, double? height, BoxFit? fit, String? blurHash, bool? showFullScreenImage, Color? color}) {
  ...
  static SystemUiOverlayStyle getSystemUiOverlayStyle({required BuildContext context, required Color statusBarColor}) {
    return SystemUiOverlayStyle(
        ...
        systemNavigationBarIconBrightness: context.watch<AppThemeCubit>().state.appTheme == AppTheme.dark ? Brightness.light : Brightness.dark,
        //
...
/*  static PreferredSize buildAppBar(BuildContext context,
  ...
  static PreferredSize buildAppBar(BuildContext context,
      {String? title, bool? showBackButton, List<Widget>? actions, List<Widget>? bottom, double? bottomHeight, bool? hideTopBorder, VoidCallback? onBackPress, Color? backgroundColor}) {
    return PreferredSize(
      preferredSize: Size.fromHeight(55 + (bottomHeight ?? 0)),
      child: Column(
        mainAxisSize: MainAxisSize.min,
          ...
          Expanded(
            child: RoundedBorderOnSomeSidesWidget(
              ...
              child: Container(
                ...
                child: Padding(
                  padding: EdgeInsets.symmetric(horizontal: (showBackButton ?? false) ? 0 : 20, vertical: (showBackButton ?? false) ? 0 : 18),
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    ...
                    children: [
                      ...
                      Text(
                        title ?? "",
                      ).color(context.color.textDefaultColor).bold(weight: FontWeight.w600).size(18),
                      if (actions != null) ...[const Spacer(), ...actions],
                    ],
                  ),
                ),
              ...
              if (isInProgress != true) ...[
                Flexible(
                  child: Text(
                    ...
                    softWrap: true,
                  ).color(textColor ?? context.color.buttonColor).size(fontSize ?? context.font.larger).centerAlign(),
                ),
              ] else ...[
                if (showProgressTitle ?? false)
                  Flexible(
                    child: Text(
                      ...
                      overflow: TextOverflow.ellipsis,
                      softWrap: true,
                    ).color(context.color.buttonColor).size(fontSize ?? context.font.larger).centerAlign(),
                  ),
              ]
            ],
          ),
  ...
  static void noCoinDialog(BuildContext context, Function() function) async {
    UiUtils.showBlurredDialoge(
      context,
      dialoge: BlurredDialogBox(
        title: 'Not enough coins'.translate(context),
        acceptButtonName: 'Add coins'.translate(context),
        cancelButtonName: 'cancelLbl'.translate(context),
        acceptButtonColor: context.color.territoryColor,
        acceptTextColor: context.color.secondaryColor,
        content: StatefulBuilder(builder: (context, update) {
          return Text('Please add your Bid Coins to use this function');
        }),
        isAcceptContainesPush: false,
        onAccept: () async {
          function.call();
  ...
  static Future showBlurredDialoge(BuildContext context, {required BlurDialoge dialoge, double? sigmaX, double? sigmaY}) async {
...
// Counting the perceptive luminance - human eye favors green color...
  ...
  static infoPriceCoin(BuildContext context, {BidcoinsPackageModel? data}) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: false,
      backgroundColor: context.color.primaryColor.darken(-5),
            ...
            Container(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    UiUtils.getSvg(AppIcons.bcaLogo, height: 20),
                    const SizedBox(
                      height: 8,
                    ),
                    Text("CV Lelangpedia Indonesia Emas").size(context.font.normal),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text("7645818989").size(context.font.normal),
                        GestureDetector(
                            onTap: () async {
                              await HapticFeedback.vibrate();
                              Navigator.of(context).pop();
                              var clipboardData = ClipboardData(text: "101 101 101");
                              Clipboard.setData(clipboardData).then((_) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text("Salin".translate(context)),
                                  ),
                                );
                              });
                            },
                            child: Text("Salin").size(context.font.normal).color(territoryColor_)),
                      ],
                    ),
                    const SizedBox(
                      height: 8,
                    ...
                    Text("total Bid Coins = ${UiUtils().numberFormat(amount: data?.bidcoin)}").size(context.font.small),
                    const SizedBox(
                      height: 8,
                    ),
                    Text("Total Paid").size(context.font.small),
                    Text("Rp. ${UiUtils().numberFormat(amount: data?.price)}").size(context.font.extraLarge).bold(),
                  ],
                ),
              ),
            ),
