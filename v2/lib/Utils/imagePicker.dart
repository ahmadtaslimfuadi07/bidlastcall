...
class PickImage {
  ...
  pick({ImageSource? source, bool? pickMultiple, int? imageLimit, int? maxLength, required BuildContext context}) async {
    if (pickMultiple == false || pickMultiple == null) {
      await _picker
          .pickImage(
        source: source ?? ImageSource.gallery,
      ...
      List<XFile> list = await _picker.pickMultiImage(imageQuality: Constant.uploadImageQuality, requestFullMetadata: true);

      if (imageLimit != null && maxLength != null && (list.length + maxLength) > imageLimit) {
        HelperUtils.showSnackBarMessage(context, "max5ImagesAllowed".translate(context));
      } else {
        Iterable<Future<File>> result = list.map((image) async {
          File myImage = File(image.path);
          if (await myImage.length() > Constant.maxSizeInBytes) {
