...
class CustomImageHeroAnimation extends StatefulWidget {
  ...
  const CustomImageHeroAnimation({super.key, required this.child, required this.type, this.image});
...
class _CustomHeroDestinationScreen extends StatefulWidget {
  ...
  final CImageType type;
  final dynamic image;
  const _CustomHeroDestinationScreen({required this.renderWidgetData, required this.type, this.image});

  @override
  State<_CustomHeroDestinationScreen> createState() => _CustomHeroDestinationScreenState();
}

class _CustomHeroDestinationScreenState extends State<_CustomHeroDestinationScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    var targetPosition = Offset((mediaQuery.width / 2) - (width / 2), (mediaQuery.height / 2) - (height / 2));

    _sizeTween = Tween(begin: Offset(width, height), end: const Offset(200, 200)).animate(_controller);
    Future.delayed(
      Duration.zero,
      () {
        _positionTween = Tween(
                begin: Offset(widget.renderWidgetData['x'], widget.renderWidgetData['y']),
                end: Offset((MediaQuery.of(context).size.width / 2) - (200 / 2), (MediaQuery.of(context).size.height / 2) - 100))
            .animate(_controller);
      },
    );
