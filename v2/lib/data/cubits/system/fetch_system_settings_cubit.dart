...
class FetchSystemSettingsCubit extends Cubit<FetchSystemSettingsState> with HydratedMixin {
  ...
  Future<void> fetchSettings({bool? forceRefresh}) async {
    try {
      ...
      if (forceRefresh == true) {
        ...
        Constant.currencySymbol = _getSetting(settings, SystemSetting.currencySymball);
        Constant.maintenanceMode = _getSetting(settings, SystemSetting.maintenanceMode);
        Constant.isGoogleBannerAdsEnabled = _getSetting(settings, SystemSetting.bannerAdStatus);
        Constant.isGoogleInterstitialAdsEnabled = _getSetting(settings, SystemSetting.interstitialAdStatus);
        Constant.bannerAdIdAndroid = _getSetting(settings, SystemSetting.bannerAdAndroidAd) ?? "";
        Constant.bannerAdIdIOS = _getSetting(settings, SystemSetting.bannerAdiOSAd) ?? "";
        Constant.interstitialAdIdAndroid = _getSetting(settings, SystemSetting.interstitialAdAndroidAd) ?? "";
        Constant.interstitialAdIdIOS = _getSetting(settings, SystemSetting.interstitialAdiOSAd) ?? "";
        ...
        if (state is! FetchSystemSettingsSuccess) {
          ...
          await CheckInternet.check(
            ...
            onNoInternet: () {
              emit(FetchSystemSettingsSuccess(settings: (state as FetchSystemSettingsSuccess).settings));
            },
          );
        }
      }
  ...
  dynamic getSetting(SystemSetting selected) {
    if (state is FetchSystemSettingsSuccess) {
      ...
      var selectedSettingData = (settings[Constant.systemSettingKeys[selected]]);
  ...
  dynamic _getSetting(Map settings, SystemSetting selected) {
    var selectedSettingData = settings['data'][Constant.systemSettingKeys[selected]];
  ...
  FetchSystemSettingsState? fromJson(Map<String, dynamic> json) {
    try {
      if (json['cubit_state'] == "FetchSystemSettingsSuccess") {
        FetchSystemSettingsSuccess fetchSystemSettingsSuccess = FetchSystemSettingsSuccess.fromMap(json);
