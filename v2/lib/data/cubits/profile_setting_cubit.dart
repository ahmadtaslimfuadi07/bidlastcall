...
class ProfileSettingCubit extends Cubit<ProfileSettingState> with HydratedMixin {
  ProfileSettingCubit() : super(ProfileSettingInitial());

  void fetchProfileSetting(BuildContext context, String title, {bool? forceRefresh}) async {
  ...
  Future<String?> fetchProfileSettingFromDb(BuildContext context, String title) async {
    try {
      String? profileSettingData;
      ...
      if (!response[Api.error]) {
        ...
        if (title == Api.maintenanceMode) {
          ...
          if (title == Api.termsAndConditionsSeller) {
            profileSettingData = data['terms_conditions_seller'];
            // .where((element) => element['type'] == "terms_conditions")
            // .first['data'];
          }
  ...
  ProfileSettingState? fromJson(Map<String, dynamic> json) {
    try {
      if (json['cubit_state'] == "ProfileSettingFetchSuccess") {
        ProfileSettingFetchSuccess profileSettingFetchSuccess = ProfileSettingFetchSuccess.fromMap(json);
