...
class LoadChatMessagesCubit extends Cubit<LoadChatMessagesState> with HydratedMixin {
  ...
  Future<void> loadMore() async {
    try {
      if (state is LoadChatMessagesSuccess) {
        ...
        DataOutput<ChatMessage> result = await _chatRepostiory.getMessagesApi(page: (state as LoadChatMessagesSuccess).currentPage + 1, itemOfferId: (state as LoadChatMessagesSuccess).itemOfferId);

        LoadChatMessagesSuccess messagesSuccessState = (state as LoadChatMessagesSuccess);
  ...
  bool hasMoreChat() {
    if (state is LoadChatMessagesSuccess) {
      return (state as LoadChatMessagesSuccess).messages.length < (state as LoadChatMessagesSuccess).totalPage;
    }
    return false;
  }
