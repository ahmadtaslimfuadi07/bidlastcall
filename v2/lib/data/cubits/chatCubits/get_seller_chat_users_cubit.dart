...
class GetSellerChatListCubit extends Cubit<GetSellerChatListState> with HydratedMixin {
  ...
  void fetch() async {
    try {
      ...
      DataOutput<ChatedUser> result = await _chatRepostiory.fetchSellerChatList(1);

      emit(
        GetSellerChatListSuccess(isLoadingMore: false, hasError: false, chatedUserList: result.modelList, total: result.total, page: 1),
      );
    } catch (e) {
  ...
  void addNewChat(ChatedUser user) {
    ...
    if (state is GetSellerChatListSuccess) {
      List<ChatedUser> chatedUserList = (state as GetSellerChatListSuccess).chatedUserList;
      bool contains = chatedUserList.any(
        (element) => element.sellerId == user.sellerId,
      );
      if (contains == false) {
        chatedUserList.insert(0, user);
        emit((state as GetSellerChatListSuccess).copyWith(chatedUserList: chatedUserList));
      }
    }
  }
  ...
  Future<void> loadMore() async {
    try {
      if (state is GetSellerChatListSuccess) {
        ...
        DataOutput<ChatedUser> result = await _chatRepostiory.fetchSellerChatList(
          (state as GetSellerChatListSuccess).page + 1,
        );

        GetSellerChatListSuccess messagesSuccessState = (state as GetSellerChatListSuccess);
      ...
      emit((state as GetSellerChatListSuccess).copyWith(isLoadingMore: false, hasError: true));
    }
  }

  bool hasMoreData() {
    if (state is GetSellerChatListSuccess) {
      return (state as GetSellerChatListSuccess).chatedUserList.length < (state as GetSellerChatListSuccess).total;
    }
