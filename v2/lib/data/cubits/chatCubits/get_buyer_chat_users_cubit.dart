...
class GetBuyerChatListCubit extends Cubit<GetBuyerChatListState> with HydratedMixin {
  ...
  void fetch() async {
    try {
      ...
      DataOutput<ChatedUser> result = await _chatRepostiory.fetchBuyerChatList(1);

      emit(
        GetBuyerChatListSuccess(isLoadingMore: false, hasError: false, chatedUserList: result.modelList, total: result.total, page: 1),
      );
    } catch (e) {
  ...
  void addNewChat(ChatedUser user) {
    ...
    if (state is GetBuyerChatListSuccess) {
      List<ChatedUser> chatedUserList = (state as GetBuyerChatListSuccess).chatedUserList;
      bool contains = chatedUserList.any(
        (element) => element.itemId == user.itemId,
      );
      if (contains == false) {
        chatedUserList.insert(0, user);
        emit((state as GetBuyerChatListSuccess).copyWith(chatedUserList: chatedUserList));
      }
    }
  }
  ...
  Future<void> loadMore() async {
    try {
      if (state is GetBuyerChatListSuccess) {
        ...
        DataOutput<ChatedUser> result = await _chatRepostiory.fetchBuyerChatList(
          (state as GetBuyerChatListSuccess).page + 1,
        );

        GetBuyerChatListSuccess messagesSuccessState = (state as GetBuyerChatListSuccess);
      ...
      emit((state as GetBuyerChatListSuccess).copyWith(isLoadingMore: false, hasError: true));
    }
  }

  bool hasMoreData() {
    if (state is GetBuyerChatListSuccess) {
      return (state as GetBuyerChatListSuccess).chatedUserList.length < (state as GetBuyerChatListSuccess).total;
    }
...
/*  bool isItemOffer(int itemId) {
  ...
  ChatedUser? getOfferForItem(int itemId) {
    if (state is GetBuyerChatListSuccess) {
      List<ChatedUser> offerList = (state as GetBuyerChatListSuccess).chatedUserList;
