...
import '../../Repositories/Location/states_repository.dart';
...
class FetchStatesSuccess extends FetchStatesState {
  ...
  FetchStatesSuccess({required this.isLoadingMore, required this.loadingMoreError, required this.statesModel, required this.page, required this.total, this.countryId});

  FetchStatesSuccess copyWith({bool? isLoadingMore, bool? loadingMoreError, List<StatesModel>? statesModel, int? page, int? total, int? countryId}) {
...
class FetchStatesCubit extends Cubit<FetchStatesState> {
  ...
  Future<void> fetchStates({
    ...
    try {
      ...
      DataOutput<StatesModel> result = await _statesRepository.fetchStates(countryId: countryId, page: 1, search: search);
      emit(
        FetchStatesSuccess(
          isLoadingMore: false,
  ...
  Future<void> fetchStatesMore({required int countryId}) async {
    try {
      if (state is FetchStatesSuccess) {
        ...
        DataOutput<StatesModel> result = await _statesRepository.fetchStates(countryId: countryId, page: (state as FetchStatesSuccess).page + 1);
  ...
  bool hasMoreData() {
    if (state is FetchStatesSuccess) {
      return (state as FetchStatesSuccess).statesModel.length < (state as FetchStatesSuccess).total;
    }
    return false;
  }
}
