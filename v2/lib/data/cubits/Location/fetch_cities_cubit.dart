...
import '../../Repositories/Location/cities_repository.dart';
...
class FetchCitiesSuccess extends FetchCitiesState {
  ...
  FetchCitiesSuccess({required this.isLoadingMore, required this.loadingMoreError, required this.citiesModel, required this.page, required this.total, this.stateId});

  FetchCitiesSuccess copyWith({bool? isLoadingMore, bool? loadingMoreError, List<CityModel>? citiesModel, int? page, int? total, int? stateId}) {
...
class FetchCitiesCubit extends Cubit<FetchCitiesState> {
  ...
  Future<void> fetchCities({required int stateId, String? search}) async {
    try {
      ...
      DataOutput<CityModel> result = await _citiesRepository.fetchCities(stateId: stateId, page: 1, search: search);
      emit(
        FetchCitiesSuccess(
          isLoadingMore: false,
  ...
  Future<void> fetchCitiesMore({required int stateId}) async {
    try {
      if (state is FetchCitiesSuccess) {
        ...
        DataOutput<CityModel> result = await _citiesRepository.fetchCities(stateId: stateId, page: (state as FetchCitiesSuccess).page + 1);
  ...
  bool hasMoreData() {
    if (state is FetchCitiesSuccess) {
      return (state as FetchCitiesSuccess).citiesModel.length < (state as FetchCitiesSuccess).total;
    }
    return false;
  }
}
