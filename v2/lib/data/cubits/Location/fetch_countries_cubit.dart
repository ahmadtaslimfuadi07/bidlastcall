...
import '../../Repositories/Location/countries_repository.dart';
...
class FetchCountriesCubit extends Cubit<FetchCountriesState> {
  ...
  Future<void> fetchCountries({required String search}) async {
    try {
      ...
      DataOutput<CountriesModel> result = await _countriesRepository.fetchCountries(page: 1, search: search);
      emit(
        FetchCountriesSuccess(
          isLoadingMore: false,
  ...
  Future<void> fetchCountriesMore() async {
    try {
      if (state is FetchCountriesSuccess) {
        ...
        DataOutput<CountriesModel> result = await _countriesRepository.fetchCountries(page: (state as FetchCountriesSuccess).page + 1);
  ...
  bool hasMoreData() {
    if (state is FetchCountriesSuccess) {
      return (state as FetchCountriesSuccess).countriesModel.length < (state as FetchCountriesSuccess).total;
    }
    return false;
  }
}
