...
import '../../Repositories/Location/areas_repository.dart';
...
class FetchAreasSuccess extends FetchAreasState {
  ...
  FetchAreasSuccess({required this.isLoadingMore, required this.loadingMoreError, required this.areasModel, required this.page, required this.total, this.cityId});

  FetchAreasSuccess copyWith({bool? isLoadingMore, bool? loadingMoreError, List<AreaModel>? areasModel, int? page, int? total, int? cityId}) {
...
class FetchAreasCubit extends Cubit<FetchAreasState> {
  ...
  Future<void> fetchAreas({required int cityId, String? search}) async {
    try {
      ...
      DataOutput<AreaModel> result = await _areasRepository.fetchAreas(cityId: cityId, page: 1, search: search);
      emit(
        FetchAreasSuccess(
          isLoadingMore: false,
  ...
  Future<void> fetchAreasMore({required int cityId}) async {
    try {
      if (state is FetchAreasSuccess) {
        ...
        DataOutput<AreaModel> result = await _areasRepository.fetchAreas(cityId: cityId, page: (state as FetchAreasSuccess).page + 1);
  ...
  bool hasMoreData() {
    if (state is FetchAreasSuccess) {
      return (state as FetchAreasSuccess).areasModel.length < (state as FetchAreasSuccess).total;
    }
    return false;
  }
}
