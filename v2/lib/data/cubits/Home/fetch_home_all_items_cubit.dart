...
class FetchHomeAllItemsSuccess extends FetchHomeAllItemsState {
  ...
  FetchHomeAllItemsSuccess({required this.items, required this.isLoadingMore, required this.loadingMoreError, required this.page, required this.total});
...
class FetchHomeAllItemsCubit extends Cubit<FetchHomeAllItemsState> with HydratedMixin {
  ...
  void fetch({String? city, int? areaId}) async {
    try {
      ...
      DataOutput<ItemModel> result = await _homeRepository.fetchHomeAllItems(page: 1, city: city, areaId: areaId);
  ...
  Future<void> fetchMore({String? city}) async {
    try {
      if (state is FetchHomeAllItemsSuccess) {
        ...
        DataOutput<ItemModel> result = await _homeRepository.fetchHomeAllItems(page: (state as FetchHomeAllItemsSuccess).page + 1, city: city);

        FetchHomeAllItemsSuccess itemModelState = (state as FetchHomeAllItemsSuccess);
        itemModelState.items.addAll(result.modelList);
        emit(FetchHomeAllItemsSuccess(isLoadingMore: false, loadingMoreError: false, items: itemModelState.items, page: (state as FetchHomeAllItemsSuccess).page + 1, total: result.total));
      }
    } catch (e) {
      emit((state as FetchHomeAllItemsSuccess).copyWith(isLoadingMore: false, loadingMoreError: true));
    }
  }

  bool hasMoreData() {
    if (state is FetchHomeAllItemsSuccess) {
      return (state as FetchHomeAllItemsSuccess).items.length < (state as FetchHomeAllItemsSuccess).total;
    }
    return false;
  }
