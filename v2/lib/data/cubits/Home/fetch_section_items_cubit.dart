...
class FetchSectionItemsSuccess extends FetchSectionItemsState {
  ...
  FetchSectionItemsSuccess({required this.items, required this.isLoadingMore, required this.loadingMoreError, required this.page, required this.total});
...
class FetchSectionItemsCubit extends Cubit<FetchSectionItemsState> with HydratedMixin {
  ...
  void fetchSectionItem(int sectionId) async {
    try {
      ...
      DataOutput<ItemModel> result = await _homeRepository.fetchSectionItems(page: 1, sectionId: sectionId);
  ...
  Future<void> fetchSectionItemMore(int sectionId) async {
    try {
      if (state is FetchSectionItemsSuccess) {
        ...
        DataOutput<ItemModel> result = await _homeRepository.fetchSectionItems(page: (state as FetchSectionItemsSuccess).page + 1, sectionId: sectionId);

        FetchSectionItemsSuccess itemModelState = (state as FetchSectionItemsSuccess);
        itemModelState.items.addAll(result.modelList);
        emit(FetchSectionItemsSuccess(isLoadingMore: false, loadingMoreError: false, items: itemModelState.items, page: (state as FetchSectionItemsSuccess).page + 1, total: result.total));
      }
    } catch (e) {
      emit((state as FetchSectionItemsSuccess).copyWith(isLoadingMore: false, loadingMoreError: true));
    }
  }

  bool hasMoreData() {
    if (state is FetchSectionItemsSuccess) {
      return (state as FetchSectionItemsSuccess).items.length < (state as FetchSectionItemsSuccess).total;
    }
    return false;
  }
