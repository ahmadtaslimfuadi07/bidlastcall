...
class FetchSubCategoriesSuccess extends FetchSubCategoriesState {
  ...
  factory FetchSubCategoriesSuccess.fromJson(String source) => FetchSubCategoriesSuccess.fromMap(json.decode(source) as Map<String, dynamic>);
...
class FetchSubCategoriesCubit extends Cubit<FetchSubCategoriesState> with HydratedMixin {
  FetchSubCategoriesCubit() : super(FetchSubCategoriesInitial());

  final CategoryRepository _categoryRepository = CategoryRepository();

  Future<void> fetchSubCategories({bool? forceRefresh, bool? loadWithoutDelay, required int categoryId}) async {
    try {
      emit(FetchSubCategoriesInProgress());

      DataOutput<CategoryModel> categories = await _categoryRepository.fetchCategories(page: 1, categoryId: categoryId);

      emit(FetchSubCategoriesSuccess(total: categories.total, categories: categories.modelList, page: 1, hasError: false, isLoadingMore: false));
    } catch (e) {
  ...
  Future<void> fetchSubCategoriesMore() async {
    try {
      if (state is FetchSubCategoriesSuccess) {
        ...
        emit((state as FetchSubCategoriesSuccess).copyWith(isLoadingMore: true));
        DataOutput<CategoryModel> result = await _categoryRepository.fetchCategories(
          page: (state as FetchSubCategoriesSuccess).page + 1,
        );

        FetchSubCategoriesSuccess categoryState = (state as FetchSubCategoriesSuccess);
        ...
        emit(FetchSubCategoriesSuccess(isLoadingMore: false, hasError: false, categories: categoryState.categories, page: (state as FetchSubCategoriesSuccess).page + 1, total: result.total));
      }
    } catch (e) {
      emit((state as FetchSubCategoriesSuccess).copyWith(isLoadingMore: false, hasError: true));
    }
  }

  bool hasMoreData() {
    if (state is FetchSubCategoriesSuccess) {
      return (state as FetchSubCategoriesSuccess).categories.length < (state as FetchSubCategoriesSuccess).total;
    }
    return false;
  }
