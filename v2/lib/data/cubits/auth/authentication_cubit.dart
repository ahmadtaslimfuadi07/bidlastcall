...
class AuthenticationCubit extends Cubit<AuthenticationState> {
  ...
  MMultiAuthentication mMultiAuthentication = MMultiAuthentication({"google": GoogleLogin(), "email": EmailLogin(), if (Platform.isIOS) "apple": AppleLogin(), "phone": PhoneLogin()});

  void init() {
    mMultiAuthentication.init();
  }

  void setData({required LoginPayload payload, required AuthenticationType type}) {
    this.type = type;
  ...
  void authenticate() async {
    ...
    try {
      ...
      if (payloadData is EmailLoginPayload && payloadData.type == EmailLoginType.login) {
        if (credential != null) {
          User? user = credential.user;
          if (user != null && !user.emailVerified) {
            // Handle the case when the user's email is not verified
            emit(AuthenticationFail(HelperUtils.showSnackBarMessage(Constant.navigatorKey.currentContext, "pleaseFirstVerifyUser".translate(Constant.navigatorKey.currentContext!))));
          } else {
