...
class LoginCubit extends Cubit<LoginState> {
  ...
  void login({String? phoneNumber, required String firebaseUserId, required String type, required UserCredential credential, String? countryCode}) async {
    try {
      ...
      if ((result['data']['name'] == "" || result['data']['name'] == null) ||
          ...
          (result['data']['mobile'] == "" || result['data']['mobile'] == null)) {
        HiveUtils.setProfileNotCompleted();

        var data = result['data'];
        //data['countryCode'] = countryCode;
        HiveUtils.setUserData(data);
        emit(LoginSuccess(apiResponse: Map<String, dynamic>.from(result['data']), isProfileCompleted: false, credential: credential));
      } else {
        var data = result['data'];
        //data['countryCode'] = countryCode;
        HiveUtils.setUserData(data);
        emit(LoginSuccess(apiResponse: Map<String, dynamic>.from(result['data']), isProfileCompleted: true, credential: credential));
      }
    } catch (e) {
