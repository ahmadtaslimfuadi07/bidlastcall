...
class FetchMyItemsSuccess extends FetchMyItemsState {
  ...
  FetchMyItemsSuccess({required this.total, required this.page, required this.isLoadingMore, required this.hasError, required this.getItemsWithStatus, required this.items});
...
class FetchMyItemsCubit extends Cubit<FetchMyItemsState> {
  ...
  void fetchMyItems({String? getItemsWithStatus}) async {
    try {
      ...
      emit(FetchMyItemsSuccess(hasError: false, isLoadingMore: false, page: 1, items: result.modelList, total: result.total, getItemsWithStatus: getItemsWithStatus));
    } catch (e) {
  ...
  bool hasMoreData() {
    if (state is FetchMyItemsSuccess) {
      return (state as FetchMyItemsSuccess).items.length < (state as FetchMyItemsSuccess).total;
    }
    return false;
  }
}
