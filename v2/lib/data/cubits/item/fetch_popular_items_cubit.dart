...
class FetchPopularItemsSuccess extends FetchPopularItemsState {
  ...
  FetchPopularItemsSuccess({required this.total, required this.page, required this.isLoadingMore, required this.hasError, required this.sortBy, required this.items});
...
class FetchPopularItemsCubit extends Cubit<FetchPopularItemsState> {
  ...
  void fetchPopularItems() async {
    try {
      ...
      DataOutput<ItemModel> result = await _itemRepository.fetchPopularItems(sortBy: "popular_items", page: 1);
      emit(FetchPopularItemsSuccess(hasError: false, isLoadingMore: false, page: 1, items: result.modelList, total: result.total, sortBy: "popular_items"));
    } catch (e) {
  ...
  Future<void> fetchMyMoreItems() async {
    try {
      if (state is FetchPopularItemsSuccess) {
        ...
        FetchPopularItemsSuccess myItemsState = (state as FetchPopularItemsSuccess);
  ...
  bool hasMoreData() {
    if (state is FetchPopularItemsSuccess) {
      return (state as FetchPopularItemsSuccess).items.length < (state as FetchPopularItemsSuccess).total;
    }
    return false;
  }
}
