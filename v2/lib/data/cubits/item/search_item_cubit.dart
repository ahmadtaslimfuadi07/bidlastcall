...
class SearchItemSuccess extends SearchItemState {
  ...
  SearchItemSuccess({required this.searchQuery, required this.total, required this.page, required this.isLoadingMore, required this.hasError, required this.searchedItems, required this.hasMore});
  ...
  Future<void> searchItem(
    ...
    try {
      ...
      emit(SearchItemSuccess(
          searchQuery: query, total: result.total, hasError: false, isLoadingMore: false, page: page, searchedItems: result.modelList, hasMore: (result.modelList.length < result.total)));
    } catch (e) {
      if (e.toString() == "No Data Found") {
        //incase of 0 Favorite length - make it success for fresh users

        emit(SearchItemSuccess(searchQuery: query, total: 0, hasError: false, isLoadingMore: false, page: page, searchedItems: [], hasMore: false));
      } else {
