...
class ManageItemCubit extends Cubit<ManageItemState> {
  ...
  void manage(ManageItemType type, Map<String, dynamic> data, File? mainImage, List<File>? otherImage) async {
    try {
      emit(ManageItemInProgress());

      if (type == ManageItemType.add) {
        ItemModel itemModel = await _itemRepository.createItem(data, mainImage!, otherImage!);
        emit(ManageItemSuccess(itemModel, type));
      } else if (type == ManageItemType.edit) {
        ItemModel itemModel = await _itemRepository.editItem(data, mainImage, otherImage);
        emit(ManageItemSuccess(itemModel, type));
      }
    } catch (e) {
