...
class PersonalizedInterestSettings {
  ...
  factory PersonalizedInterestSettings.fromMap(Map<String, dynamic> map) {
    return PersonalizedInterestSettings(
      ...
      categoryIds: (map['category_ids'] is String) ? [] : ((map['category_ids']) as List).forceInt(),
      priceRange: (map['price_range'] is String) ? [0, 50] : (map['price_range'] as List).forceDouble(),
      itemType: (map['item_type'] is String) ? [] : (map['item_type'] as List).forceInt(),
      outdoorFacilityIds: (map['outdoor_facilitiy_ids'] is String) ? [] : ((map['outdoor_facilitiy_ids'] ?? []) as List).forceInt(),
      city: map['city'] ?? '',
    );
  }
