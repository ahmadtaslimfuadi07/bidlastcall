...
class ItemRepository {
  Future<String> buyNowItem(
    Map<String, dynamic> param,
  ) async {
    try {
      Map<String, dynamic> response = await Api.post(
        url: Api.postBuyNow,
        parameter: param, /* useAuthToken: true*/
      );

      return response['message'];
    } catch (e) {
      debugPrint("error add Item ${e.toString()}");
      rethrow;
    }
  }
  ...
  Future<ItemModel> createItem(
    ...
    try {
      ...
      MultipartFile image = await MultipartFile.fromFile(mainImage.path, filename: path.basename(mainImage.path));
      ...
      if (otherImages != null && otherImages.isNotEmpty) {
        List<Future<MultipartFile>> futures = otherImages.map((imageFile) {
          ...
          return MultipartFile.fromFile(imageFile.path, filename: path.basename(imageFile.path));
        }).toList();
  ...
  Future<DataOutput<ItemModel>> fetchMyFeaturedItems({int? page}) async {
    try {
      ...
      List<ItemModel> itemList = (response['data']['data'] as List).map((element) => ItemModel.fromJson(element)).toList();

      return DataOutput(total: response['data']['total'] ?? 0, modelList: itemList);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<DataOutput<ItemModel>> fetchItemFromItemId(int id) async {
    ...
    List<ItemModel> modelList = (response['data'] as List).map((e) => ItemModel.fromJson(e)).toList();
  ...
  Future<Map> changeMyItemStatus({required int itemId, required String status}) async {
  ...
  Future<DataOutput<ItemModel>> fetchItemFromCatId({required int categoryId, required int page, String? search, String? sortBy, ItemFilterModel? filter}) async {
    Map<String, dynamic> parameters = {
    ...
    // var response2 = await Api.customeUrl(url: 'https://ahmadtaslimfuadi07.github.io/jsonlottie/price.txt');

    // print("repomse text : ${response2} -- ${response2.runtimeType} ");
  ...
  Future<void> itemTotalClick(int id) async {
    Map<String, dynamic> response = await Api.post(url: Api.setItemTotalClickApi, parameter: {Api.itemId: id});
  }

  Future<Map> makeAnOfferItem(int id, int amount) async {
    Map response = await Api.post(url: Api.itemOfferApi, parameter: {Api.itemId: id, Api.amount: amount});
    return response;
  }
