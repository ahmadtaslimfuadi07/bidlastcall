...
class ReportItemRepository {
  ...
  Future<Map> reportItem({required int reasonId, required int itemId, String? message}) async {
    try {
      Map response = await Api.post(
        url: Api.addReportsApi,
        parameter: {"report_reason_id": (reasonId == -10) ? "" : reasonId, "item_id": itemId, if (message != null) "other_message": message},
      );
